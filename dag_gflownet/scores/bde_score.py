...
class BDeScore(BaseScore):
    ...
    def get_local_scores(self, target, indices, indices_after=None):
        ...
        #modified to include target, indices
    ...
    #target, indices are contained in key!
    def local_score(self, key, counts):
        ...
        if self.prior.adjacency_prior == False:
            prior = self.prior(num_parents)
        else:
            #Added -- compute prior from (target, (indices))
            #calls self.prior.log_prior
            #print(key)
            if key is not None:
